...
describe LinkChecker do
  ...
  describe "checks links and" do
    ...
    it "declares good links to be good." do
      expect(LinkChecker.check_uri(@good_uri)).to be_a(LinkChecker::Good)
    end

    it "declares bad links to be bad." do
      expect(LinkChecker.check_uri(@bad_uri)).to be_a(LinkChecker::Error)
    end
    ...
    describe "follows redirects to the destination and" do
      ...
      it "declares good redirect targets to be good." do
        ...
        expect(result).to be_a(LinkChecker::Redirect)
        expect(result.final_destination_uri_string).to be == @good_uri.to_s
      end
      ...
      it "declares bad redirect targets to be bad." do
        ...
        result = LinkChecker.check_uri(@redirect_uri)
        expect(result).to be_a(LinkChecker::Error)
      end
    ...
    describe "follow relative redirects for and" do
      ...
      it "declares the final target to be good." do
        ...
        expect(result).to be_a(LinkChecker::Redirect)
        expect(result.final_destination_uri_string).to be == destination.to_s
      end
    end
  ...
  describe "scans a file path and prints output" do
    ...
    it "prints red when the links are bad." do
      ...
      expect($stdout).to receive(:puts).with(/Problem\: .*\.html/).once
      expect($stdout).to receive(:puts).with(/Link\: http/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Response/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Checked 20 links in 1 HTML file and found 20 errors/)
      expect(LinkChecker.new(:target => @site_path).check_uris).to be == 1 # Return value: error
    end
    ...
    it "prints yellow warnings when the links redirect." do
      ...
      expect($stdout).to receive(:puts).with(/Checked\: .*\.html/).once
      expect($stdout).to receive(:puts).with(/Warning/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Redirected/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Checked 20 links in 1 HTML file and found no errors/)
      expect(LinkChecker.new(:target => @site_path).check_uris).to be == 0 # Return value: good
    end
    ...
    it "prints errors when there are warnings with the --warnings_are_errors option." do
      ...
      expect($stdout).to receive(:puts).with(/Problem\: .*\.html/).once
      expect($stdout).to receive(:puts).with(/Link/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Redirected/).exactly(20).times
      expect($stdout).to receive(:puts).with(/Checked 20 links in 1 HTML file and found 20 errors/)
      expect(LinkChecker.new(
        :target => @site_path,
        :options => { :warnings_are_errors => true }
      ).check_uris).to be == 1 # Return value: error
    end
  ...
  describe "prints output for invalid links and" do
    ...
    it "declares them to be bad." do
      allow(LinkChecker).to receive(:external_link_uri_strings).and_return(
        ['hQQp://!!!.com', 'hOOp://???.com'])
      expect($stdout).to receive(:puts).with(/Problem\: .*\.html/).once
      expect($stdout).to receive(:puts).with(/Link/).twice
      expect($stdout).to receive(:puts).with(/Response/).twice
  ...
  it "crawls a web site." do
    ...
    allow(LinkChecker).to receive(:external_link_uri_strings).and_return(['http://something.com'])
    allow(LinkChecker).to receive(:check_uri) do
      LinkChecker::Good.new(:uri_string => 'http://something.com')
    end
    expect($stdout).to receive(:puts).with(/Checked\: http/).once
    expect($stdout).to receive(:puts).with(/Checked 1 link in 1 HTML file and found no errors/)
  ...
  describe "produces useful return codes when" do
    ...
    it "the target file does not exist." do
      allow(Find).to receive(:find).and_raise(Errno::ENOENT.new('test'))
      expect($stdout).to receive(:puts).with(/Error/i).once
